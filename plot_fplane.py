...
parser.add_argument("fpfile", nargs="+")
parser.add_argument("geometry")
parser.add_argument("omap", help="directory if multiple args given")
...
for fpfile in args.fpfile:
	...
	if len(args.fpfile) > 1:
		name = ".".join(os.path.basename(fpfile).split(".")[:-1])
		utils.mkdir(args.omap)
		enmap.write_map(os.path.join(args.omap, name + args.suffix + ".fits"), omap)
	else:
