...
if mode == "map":
	...
	for ind in range(comm.rank, len(dirs), comm.size):
		...
		srcs[:,:2] = planet9.displace_pos(params.T[1::-1]*utils.degree, earth_pos, params.T[2], params.T[2:5]*ym*dmjd).T
	...
	from enlib import enmap, mpi, planet9, cython, bench
	...
	for ind in range(comm.rank, len(dirs), comm.size):
		...
		if args.extra_filter:
			ps, ls  = planet9.get_rough_powspec(rhs, rhs != 0)
			lknee   = planet9.estimate_lknee(ps, ls)
			np.savetxt(dirpath + "/test_spec.txt", np.array([ls,ps]).T, fmt="%15.7e", header="lknee %8.2f" % lknee)
		else:
			lknee = 0
		R       = planet9.Rmat(rhs.shape, rhs.wcs, info.beam, info.rfact, lmax=args.lmax, lknee=lknee)
		...
		# Finally characterize any residual noise under/over-estimation.
		A = planet9.get_normalization(frhs, kmap)
		frhs /= A
		kmap /= A
	...
	parser.add_argument(      "--rmin",      type=float, default=400)
	parser.add_argument(      "--rmax",      type=float, default=2000)
	parser.add_argument(      "--dr",        type=float, default=20)
	parser.add_argument(      "--vmax",      type=float, default=6.5)
	...
	if comm.rank == 0 and args.verbose:
	...
	for ti in range(comm_inter.rank, ntile, comm_inter.size):
		...
		for gind in range(comm_intra.rank, len(groups), comm_intra.size):
			...
			for i, ind in enumerate(group):
				...
				if np.any(~np.isfinite(kmap)) or np.all(kmap < 1e-10):
					if args.verbose: print("%3d skip %3d %2d %s (nan or unexposed)" % (comm.rank, gind, i, idir))
					continue
				else:
					if args.verbose: print("%3d read %3d %2d %s" % (comm.rank, gind, i, idir))
		...
		if ntot == 0:
			if comm_intra.rank == 0:
				...
				dummy_param = planet9.unpad(enmap.zeros((5,)+tshape, twcs, dtype), pad)
		...
		if comm_good.rank == 0:
			...
			param_max = enmap.zeros((5,)+sigma.shape, sigma.wcs, dtype)
		ntest = 0
		for ri, (r, vmin, vmax) in enumerate(zip(rlist, vmins, vmaxs)):
			for vy in np.arange(-nv,nv+1)*dv:
				for vx in np.arange(-nv,nv+1)*dv:
					...
					if comm_good.rank == 0:
						cython.solve(frhs_tot, kmap_tot, sigma, klim=klim)
						cython.update_total(sigma, sigma_max, param_max, hit_tot, frhs_tot, kmap_tot, r, vy, vx)
						t3 = time.time()
		...
		if comm_good.rank == 0:
			...
			mask  = kmap_tot > klim
			b, a = planet9.build_dist_map(sigma_max, mask=mask) # paralellize this
			# Find candidates above our threshold. Is returned as a list of [ra,dec,nsigma,r,vy,vx,hits].
			sigma_eff = (sigma_max-b)/a
			cands = planet9.find_candidates(sigma_eff, param_max, hit_tot, snmin=args.snmin, pad=pad//args.downgrade)
	...
	# Given a param map and a target S/N ratio, output a flux limit map
	parser = argparse.ArgumentParser()
	parser.add_argument("limit", help="dummy")
	parser.add_argument("param_map")
	parser.add_argument("odir")
	parser.add_argument("-s", "--snlim",  type=float, default=6)
	parser.add_argument("-f", "--fscale", type=float, default=1, help="Multiply fluxes by this factor")
	parser.add_argument("-e", "--escale", type=float, default=1, help="Multiply flux errors by this factor (in addition to that implied by fscale)")
	parser.add_argument("-v", "--verbose", action="store_true")
	parser.add_argument("-t", "--thin",   type=int,   default=1)
	parser.add_argument("-F", "--find",   type=float, default=0)
	args = parser.parse_args()
	import numpy as np
	from enlib import enmap, utils, cython, planet9

	ym = utils.arcmin/utils.yr2days
	if args.verbose: print("Reading %s" % (args.param_map))
	param_map = enmap.read_map(args.param_map).astype(np.float32)[...,::args.thin,::args.thin]
	r, vy, vx, frhs, kmap = param_map
	del r, vy, vx
	...
	# We can use planet9.build_dist_map to infer sigma_eff. But see #4
	#
	...
	# the upper part being steeper than the lower part. What build_dist_map does is to fit
	...
	# First apply any flux scaling to rhs and ivar. rhs has units of 1/flux and ivar 1/flux**2
	if args.verbose: print("Scaling by %.3f" % (args.fscale))
	frhs /= args.fscale    * args.escale**2
	kmap /= args.fscale**2 * args.escale**2

	# Then solve
	if args.verbose: print("Solving")
	klim        = np.percentile(kmap[::10,::10],99)*1e-3
	sigma_map   = cython.solve(frhs,     kmap, klim=klim)
	enmap.write_map(args.odir + "/test.fits", enmap.downgrade(sigma_map,20))
	dflux       = cython.solve(frhs*0+1, kmap, klim=klim)
	del frhs
	# Find the distribution correction
	if args.verbose: print("Building dist map")
	mask  = kmap > klim
	b, a  = planet9.build_dist_map(sigma_map, mask=mask)
	
	if args.find:
		# We can optionally find candidates ourselves. This lets us redo the object detection part
		# without the heavy parameter search part.
		sigma_eff = (sigma_map-b)/a
		enmap.write_map(args.odir + "/sigma_eff.fits", sigma_eff)
		cands = planet9.find_candidates(sigma_eff, param_map, snmin=args.find)
		inds  = np.argsort(cands[:,2])[::-1]
		cands = cands[inds]
		cands[:,1] = utils.rewind(cands[:,1])
		print("Found %d cands with detection statistic above %.3f" % (len(cands), args.find))
		np.savetxt(args.odir + "/cands.txt", np.array([
			cands[:,1]/utils.degree, cands[:,0]/utils.degree,
			cands[:,2], cands[:,3], cands[:,4], cands[:,5],
			cands[:,6], cands[:,7], np.full(len(cands), 1.0)]).T,
			fmt="%8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f %4.2f")

	# Typical values
	aref, bref = np.median(a[mask][::1000]), np.median(b[mask][::1000])
	dfref = np.median(kmap[mask][::1000])**-0.5
	del kmap
	print("a: %8.3f b: %8.3f ilim: %8.3f olim: %8.3f dflux: %8.3f flim: %8.3f" % (aref, bref, args.snlim, args.snlim*aref+bref, dfref, dfref*(args.snlim*aref+bref)))
	# And use it to translate our limit on sigma_eff into a flux limit
	if args.verbose: print("Building limit map")
	sigma_map_lim = np.where(mask&(a>0),args.snlim*a+b,0)
	del mask
	flux_lim      = dflux * sigma_map_lim
	if args.verbose: print("Writing %s/fluxlim.fits" % (args.odir))
	enmap.write_map(args.odir + "/fluxlim.fits", flux_lim)
